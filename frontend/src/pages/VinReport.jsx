    ...
    <div className="min-h-screen pt-32 pb-20 bg-[#050505] text-white">
      <div className="container max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          {/* Kolumna lewa: Nagłówki i korzyści */}
          <div>
          ...
          <div className="bg-[#0A0A0A] p-8 md:p-10 rounded-[2rem] border border-white/10 shadow-2xl h-fit">
            ...
            <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
              <div>
                ...
                <input type="text" maxLength={17} onChange={(e) => {setVin(e.target.value.toUpperCase()); setVinValid(VIN_REGEX.test(e.target.value.toUpperCase()))}} className="w-full bg-black border border-white/10 p-4 rounded-xl text-white focus:border-[#FFD200] outline-none" placeholder="Wpisz 17 znaków VIN" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Link do ogłoszenia</label>
                <input type="url" onChange={(e) => {setLink(e.target.value); setLinkValid(LINK_REGEX.test(e.target.value))}} className="w-full bg-black border border-white/10 p-4 rounded-xl text-white focus:border-[#FFD200] outline-none" placeholder="Wklej link (np. Otomoto, OLX)" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Twój numer telefonu</label>
                <input type="tel" onChange={(e) => {setPhone(e.target.value); setPhoneValid(PHONE_REGEX.test(e.target.value))}} className="w-full bg-black border border-white/10 p-4 rounded-xl text-white focus:border-[#FFD200] outline-none" placeholder="Wpisz numer telefonu" />
              </div>
