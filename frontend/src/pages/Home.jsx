...
const ScannerWidget = () => {
  // Referencje do elementów (bezpieczniejsze niż getElementById)
  ...
  useEffect(() => {
    // 1. Ładowanie bibliotek zewnętrznych
    ...
    // Stan skanera (wewnątrz useRef, by tick() widział aktualne wartości bez re-renderu)
    const state = {
      progress: 0,
      totalLoss: 0,
      isPaused: false,
    ...
    const setLaser = (x) => {
      ...
      if (coordTextRef.current) coordTextRef.current.textContent = pad(Math.floor(x * 42), 4);
    ...
    const triggerDefect = (defect) => {
      ...
      if (alert) {
        ...
        alert.querySelector("#alertLog").textContent = pad(state.hitSet.size, 3);
        alert.style.left = `${Math.max(22, Math.min(78, defect.pct))}%`;
        alert.classList.add("visible");
      }

      // Wycena
      const prevLoss = state.totalLoss;
      ...
      setTimeout(() => {
        ...
        state.progress += 1.5; // Mały skok, by nie zapętlić triggera
      }, 2400);
    };
    ...
    const tick = () => {
      ...
      if (!state.isPaused) {
        ...
        const breathing = 0.78 + 0.22 * Math.sin(t * Math.PI);
        state.progress += baseSpeed * breathing;
        ...
        if (state.progress >= 100) {
          ...
          state.hitSet.clear();
          state.cycle += 1;
          if (cycleRef.current) cycleRef.current.textContent = pad(state.cycle, 2);
          if (totalRef.current) {
            totalRef.current.textContent = "0 PLN";
            totalRef.current.classList.remove("text-[#FF4D4D]");
          }
        ...
        if (statusRef.current) {
          statusRef.current.textContent = "SCANNING SURFACES // LIVE";
          statusRef.current.style.color = "#F5C400";
        }

        // Sprawdzanie defektów
        for (const d of defects) {
          if (!state.hitSet.has(d.id) && Math.abs(d.pct - state.progress) < 0.5) {
            state.hitSet.add(d.id);
            triggerDefect(d);
            break;
          }
        }
  ...
  return (
    <div className="w-full flex flex-col">
      <style>{`
        ...
        .laser-line { position: absolute; top: 0; bottom: 0; width: 2px; background: #FFD200; box-shadow: 0 0 15px #FFD200; z-index: 20; transition: none; }
        ...
        .alert { position: absolute; top: 15%; width: 260px; transform: translateX(-50%) scale(0.9); opacity: 0; z-index: 50; background: rgba(10,10,12,0.9); border: 1px solid rgba(255,210,0,0.3); border-radius: 12px; backdrop-filter: blur(10px); transition: all 0.3s ease; pointer-events: none; }
        .alert.visible { opacity: 1; transform: translateX(-50%) scale(1); }
        .defect { opacity: 0; position: absolute; width: 20px; height: 20px; border: 2px solid #FF4D4D; border-radius: 50%; background: rgba(255,77,77,0.2); pointer-events: none; z-index: 15; }
      ...
      <div className="flex flex-col gap-4 w-full mb-4 z-20">
        <div className="flex items-center gap-5 px-6 py-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-2xl">
          <div className="w-16 h-16 rounded-full border-2 border-[#FFD200]/60 overflow-hidden shadow-[0_0_15px_rgba(255,210,0,0.2)]">
            <img src="https://ui-avatars.com/api/?name=Adam+Pakuła&background=111&color=FFD200&size=150" alt="Adam Pakuła" />
          </div>
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-white font-bold text-xl">Adam Pakuła</span>
              <CheckCircle className="w-5 h-5 text-[#FFD200]" />
            </div>
            <p className="text-gray-300 text-sm italic">"Chronię moich klientów przed ukrytymi wadami i kosztownymi pomyłkami."</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="flex flex-col justify-center px-6 py-4 rounded-2xl bg-[#0C0D10]/80 border border-white/10 backdrop-blur-xl">
             ...
             <div className="text-white font-bold text-lg">AUDI R8 COUPE <span className="text-[#FFD200]">V10</span></div>
          </div>
          <div className="flex items-center justify-between px-6 py-4 rounded-2xl bg-[#0C0D10]/90 border border-[#FFD200]/20 backdrop-blur-xl">
            <div className="text-right w-full">
              <div className="mono text-[10px] text-[#9AA3B2] uppercase mb-1">Wycena uszkodzeń</div>
              <div ref={totalRef} className="mono text-3xl font-black text-[#FFD200]">0 PLN</div>
              <div ref={statusRef} className="mono text-[10px] text-white/40 mt-1 uppercase">Initializing...</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* SCENNER BOX */}
      <div ref={widgetRef} className="scanner-widget" id="widget">
        ...
        <div className="car-stage">
          <model-viewer
            ...
            id="car3d"
            src="/r8.glb"
            style={{ width: '100%', height: '100%', background: 'transparent', filter: 'contrast(1.1) brightness(1.1)' }}
            exposure="1.4"
            tone-mapping="aces"
            shadow-intensity="2"
        ...
        <div ref={sparksRef} className="absolute inset-0 pointer-events-none"></div>

        {/* Floating Hud */}
        <div ref={coordHudRef} className="absolute top-[62%] z-40 pointer-events-none" style={{ transform: 'translate(-50%, -50%)' }}>
          <div className="mono text-[10px] px-2 py-1 rounded bg-black/80 text-[#FFD200] border border-white/10">
            X:<span ref={coordTextRef}>0000</span>
          </div>
        </div>

        {/* Alert Box */}
        <div ref={alertBoxRef} className="alert" id="alert">
          <div className="p-4">
            <div className="flex justify-between border-b border-white/10 pb-2 mb-2">
              <span id="alertTitle" className="text-[#FFD200] font-bold text-xs">USTERKA</span>
              <span className="text-white/40 text-[10px]">LOG#<span id="alertLog">000</span></span>
            </div>
            <div id="alertDesc" className="text-white/80 text-xs mb-3">Analizowanie...</div>
            <div className="flex justify-between items-baseline pt-2 border-t border-white/5">
              <span className="text-white/40 text-[10px]">Koszt:</span>
              <span className="text-red-500 font-bold text-lg">-<span id="alertCost">0</span> PLN</span>
            </div>
          </div>
        </div>
        ...
        <div className="absolute left-0 right-0 bottom-0 p-4 bg-gradient-to-t from-black to-transparent z-30">
          <div className="flex justify-between items-center mb-2 px-2">
            <div className="mono text-[9px] text-white/50 uppercase tracking-widest">Scanning status: <span className="text-white">Active</span></div>
            <div className="mono text-[9px] text-white/50 uppercase tracking-widest">Cycle: <span ref={cycleRef} className="text-white">01</span></div>
          </div>
          <div className="h-1 w-full bg-white/10 rounded-full overflow-hidden">
            <div ref={progressBarRef} className="h-full bg-[#FFD200] shadow-[0_0_15px_#FFD200]" style={{ width: '0%' }}></div>
          </div>
        </div>
      </div>
    </div>
        ...
        <div className="container max-w-[1500px] mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.6fr] gap-12 items-start">
            <div className="space-y-8 animate-in fade-in duration-1000">
              ...
              <button onClick={() => navigate("/kontakt")} className="group px-8 py-4 bg-[#FFD200] text-black font-black text-lg rounded-xl hover:shadow-[0_0_40px_rgba(255,210,0,0.3)] transition-all flex items-center gap-3">
            ...
            <div className="w-full lg:-mt-10">
              ...
              <div className="mt-8 flex justify-center">
                <div className="inline-flex items-center gap-3 px-6 py-2.5 rounded-full bg-white/5 border border-white/10 backdrop-blur-md text-xs font-mono text-gray-400 uppercase tracking-widest">
                   <span className="w-2 h-2 rounded-full bg-[#FFD200] animate-pulse"></span> Virtual Diagnostic active
                </div>
              </div>
            </div>
          </div>
